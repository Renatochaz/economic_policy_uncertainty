...
for (files in seq_len(length(savename))) {
    ...
    results <- results %>%
        filter(
            iee < 0.3 & epu < 0.3 & fc < 0.3
            & inv < 0.3 & invSQ < 0.3 & q_tobin < 1 &
...
for (i in seq_len(nrow(results))) {
    ...
    model <- pgmm(inv ~ stats:::lag(inv, 1:1) + stats:::lag(sq_inv, 1:1) +
        ...
        stats:::lag(inv, mod[1]:mod[2]) +
            ...
            stats:::lag(q_tobin, model_config[9]:model_config[10]) +
            stats:::lag(fbcf, model_config[11]:model_config[12]),
    data = ds,
    effect = "individual",
    model = "twosteps",
    collapse = collap,
