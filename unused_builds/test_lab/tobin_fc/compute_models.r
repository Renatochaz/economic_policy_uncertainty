...
for (i in seq_len(nrow(mat_combs))) {
    model <- tryCatch(
        pgmm(inv ~ stats:::lag(inv, 1:1) + stats:::lag(sq_inv, 1:1) +
            ...
            stats:::lag(inv, mat_combs[i, 1]:mat_combs[i, 2]) +
                ...
                stats:::lag(q_tobin, mat_combs[i, 13]:mat_combs[i, 14]),
        data = ds,
        effect = "individual",
        model = "twosteps",
        collapse = collap,
    ...
    if (skip_to_next == TRUE) {
        ...
        a <- paste0(c(
            ...
            mat_combs[i, 12], mat_combs[i, 13], mat_combs[i, 14]
        ), collapse = ",")

        results_temp <- data.frame(
            i, a,
            unname(unlist(pvalues)[25]), unname(unlist(pvalues)[26]),
            unname(unlist(pvalues)[27]), unname(unlist(pvalues)[28]),
            unname(unlist(pvalues)[29]), unname(unlist(pvalues)[30]),
...
write.csv(results_df,
    paste0("test_lab/tobin_fc/results/", collapsave, "/", savename, ".csv"),
    row.names = FALSE
)